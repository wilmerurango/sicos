...
class contrato(models.Model):
    ...
    aporte_arl = models.FloatField('Porcentaje Aportes de ARL', default=0)
    ...
    vista = models.BooleanField('¿Esta Visible ?', default=True)
...
class centro_actividad(models.Model):
    ...
    def __str__(self):
        return '%s %s %s %s %s %s %s' % (self.actividad.name_act, " - (", self.cuenta, ")"," --- ( $", '{:,}'.format(self.actividad.valor_iss), ")")
