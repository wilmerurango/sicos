...
class Reporte_SendMail_excel(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        for mail in mails:
            ...
            ws.cell(row=cont, column = 3).value = mail.fecha_envio
...
def Reporte_CppEspecialista_Pdf(request, id_):
    ...
    obj1 = contrato.objects.filter(especialista = obj2.especialista).last()
    ...
    ddd = contrato.objects.filter(especialista = obj2.especialista.id).last()
    ...
    # DEBER SER
    p.setFont('Helvetica-Bold', 6)
    p.drawString(40,y-315, "DEBER SER")
    p.line(40,y-317, 290,y-317)
    p.setFont('Helvetica', 6)
    p.drawString(40,y-323, "SALUD : " + "$ " + str('{:,}'.format(incr_aport_salud_cal)))
    p.drawString(40,y-331, "PENSION : "  + "$ " + str('{:,}'.format(incr_aport_pension_cal+incr_solida_pensional_cal)))
    p.drawString(40,y-339, "ARL : "+ "$ " + str('{:,}'.format(incr_aport_arl_cal)))
    p.drawString(40,y-347, "SEGURIDAD SOCIAL : "+ "$ " + str('{:,}'.format(incr_aport_salud_cal+incr_aport_pension_cal+incr_solida_pensional_cal)) )
    p.line(40,y-349, 290,y-349)
    ...
    for m in bbb:
        ...
        if cont_uni_cent_costo != 0:
            ...
            p.drawString(180,y-base_alt-contador_2*8+2, "Cantidad")
            p.line(180,y-base_alt-contador_2*8+2-2, 205,y-base_alt-contador_2*8+2-2)

            p.drawString(220,y-base_alt-contador_2*8+2, "Vlr ISS")
            p.line(220,y-base_alt-contador_2*8+2-2, 250,y-base_alt-contador_2*8+2-2)

            p.drawString(270,y-base_alt-contador_2*8+2, "Vlr ISS + 25%")
            p.line(270,y-base_alt-contador_2*8+2-2, 308,y-base_alt-contador_2*8+2-2)

            p.drawString(320,y-base_alt-contador_2*8+2, "Total")
            p.line(320,y-base_alt-contador_2*8+2-2, 360,y-base_alt-contador_2*8+2-2)
        ...
        for i in range(cont_uni_cent_costo):
            ...
            for j in mf:
                ...
                if j.centro_costo.name_ccos == uni_cent_costo[i] :
                    ...
                    if j.parametrizado == True :
                        p.drawString(220,y-base_alt-(contador*8), "$ " + str('{:,}'.format(j.centro_actividad.actividad.valor_iss)))
                    else:
                        p.drawString(220,y-base_alt-(contador*8), "-")
                    
                    if contador_3 == 0 :#es monto fijo?
                        if j.parametrizado == True :
                            ...
                            p.drawString(270,y-base_alt-(contador*8), "-" )
                            p.drawString(320,y-base_alt-(contador*8), "-" )
                    else:#es un tipo de evento
                        if j.parametrizado == False :
                            p.drawString(190,y-base_alt-(contador*8), "-")
                            p.drawString(320,y-base_alt-(contador*8),"$ " + str('{:,}'.format(j.valor)))
                            p.drawString(270,y-base_alt-(contador*8),"-")
                        else:
                            p.drawString(190,y-base_alt-(contador*8), str(j.valor))
                            p.drawString(320,y-base_alt-(contador*8), "$ " + str('{:,}'.format((j.centro_actividad.actividad.valor_iss*j.valor))))
                            p.drawString(270,y-base_alt-(contador*8),"-")
    ...
    if ddd.reten_arrindo == 'SI':# calcular y ubicar la retencion por concepto de ARRIENDO
        ...
        ultimo_tipo_fac = bbb.last()#EVENTO-ARRIENDO
...
BASE_DIR_MAIL = 'C:/Users/ANALISTA_COSTOS/OneDrive - FUNDACION CLINICA DEL RIO/FACT_ELECTRONICA'
...
def EnviarMail(factura, ruta):
    ...
    email = EmailMessage(
        ...
        ['facturacion_electronica@mutualser.com'],
        # cc = [] #copia de correo
    )
...
def Mail_list(request):
    ...
    return render(request, 'mail_list.html', contexto)
...
def base_solicitud(request,*args, **kwargs):
    return render(request, 'base_solicitud.html', {})
...
def especialistaCrear(request):
    if request.method == 'POST' :
        ...
        if form.is_valid(): 
            form.save()
        return redirect('especialistas_list')
    else:
...
def contrato_list(request):
    contrato1=contrato.objects.filter(vista=True)
...
def fac_especialista_detallecrear(request, id_):
    ...
    # esto se hace para capturar el ultimo resgistro e instanciarlo para el registro siguiente (mirar el else del request.method)
    ...
    ddd = contrato.objects.filter(especialista = tempo.especialista.id).last()
    ...
    if request.method == 'POST':
        ...
        if filtro_cont != 0:
            # dejar instanciado el ultimo registro diligenciado
            a = filtro.last()
            temporal=fac_especialista_detalle.objects.get(id=a.id)
            form = fac_especialista_detalleform(instance=temporal)
        else:
            # colocar el id de la factura en el detalle al iniciar el registro de los mismos
            copia_fac_especilaista_detalle = fac_especialista_detalle()
            copia_fac_especilaista_detalle.fac_especialista =  fac_especialista.objects.get(id =id_)
            form = fac_especialista_detalleform(instance=copia_fac_especilaista_detalle)
            
    
    for k in fac_especialista_detalle1:
        contrato_ins=contrato.objects.filter(especialista= k.fac_especialista.especialista.id).last()
...
def fac_especialista_detalle_elim(request, id_):
    prov = fac_especialista_detalle.objects.get(id=id_)
    eliminar_detalle_fac_esp = fac_especialista_detalle.objects.get(id=id_)
    eliminar_detalle_fac_esp.delete()
    return redirect('/crearfacdetal/'+ str(prov.fac_especialista.id) + '/')
...
def fac_especialista_detalleEdit(request, id_):
    fac_especialista_detalle1 = fac_especialista_detalle.objects.get(id=id_)
    if request.method == 'GET':
        form = fac_especialista_detalleform(instance = fac_especialista_detalle1)
    else:
        form =fac_especialista_detalleform(request.POST, instance = fac_especialista_detalle1)
        if form.is_valid():
            form.save()
        return redirect('/crearfacdetal/'+ str(fac_especialista_detalle1.fac_especialista.id) + '/')
    contexto={'form':form}
    return render(request, 'fac_especialista_detalleform.html', contexto)  
...
class fac_especialistaCrear(CreateView):
    ...
    success_url = reverse_lazy('fac_especialista_list')
...
def fac_especialista_list(request):
    ...
    for i in sum_general:
        ...
        bbb = tipo_fact.objects.all()
        ccc = centro_actividad.objects.all()
        contr = contrato.objects.filter(especialista = i.especialista).last()
        ddd = contrato.objects.filter(especialista = i.especialista).last()
        suma_parcial=factura_pdf(aaa, bbb, ccc, ddd)
...
class fac_especialistaEdit(UpdateView):
    model = fac_especialista
    form_class = fac_especialistaform
    template_name = 'fac_especialista_form.html'
    success_url = reverse_lazy('fac_especialista_list')
